...
!
            ...
            thck_wbl(i,j)=m_dwc
!
! Use a user defined, fixed thickeness of wave boundary layer.
!
            cff1=MAX( sg_zwbl(ng),1.1_r8*ksd_wbl(i,j) )   
            cff=LOG(cff1/ksd_wbl(i,j))
!
...
# endif
!
      CALL SUBROUTINE( sg_zwbl(ng), u(i,j,:), v(i,j,:),                 &
...
!
      SUBROUTINE( sg_zwbl(ng), u_1d(:), v_1d(:),                        &
     &            u_stokes_1d(:), v_stokes_1d(:), z_r_1d(:), z_w_1d(:), &
     &            ubar_1, vbar_1, ubar_stokes_1, vbar_stokes_1,         &
     &            d50, zapp_loc,                                        & 
     &            phic_sgwbl_1 )
!
!=======================================================================
...
!  Imported variable declarations.
!
      real(r8), intent(in)  :: u_1d(1:N(ng)), v_1d(1:N(ng))
      real(r8), intent(in)  :: u_stokes_1d(1:N(ng)), v_stokes_1d(1:N(ng))
      real(r8), intent(in)  :: z_r_1d(1:N(ng)), z_w_1d(0,N(ng))
      real(r8), intent(in)  :: ubar_1, vbar_1 
      real(r8), intent(in)  :: ubar_stokes_1, vbar_stokes_1
      real(r8), intent(in)  :: d50, zapp_loc
      real(r8), intent(out) :: phic_sgwbl_1
      
      real(r8) :: Zr
!    
      Zr=z_r(1)-z_w(0)
      IF ( sg_zwbl(ng).ge.Zr ) THEN
!
!  If chosen height to get near bottom-current velocity lies
!  within any vertical level, perform logarithmic interpolation.
!
        DO k=2,N(ng)
          z1=z_r(k-1)-z_w(0)
          z2=z_r(k  )-z_w(0)
          IF ( ( z1.le.sg_zwbl(ng) ).and.( sg_zwbl(ng).lt.z2 )) THEN
            fac=1.0_r8/LOG(z2/z1)
            fac1=fac*LOG(z2/sg_zwbl(ng))
            fac2=fac*LOG(sg_zwbl(ng)/z1)
!
#   ifdef BEDLOAD_VANDERA_STOKES
                Ur_sgwbl=fac1*( u(k-1)+u_stokes(k-1) )+                 &
     &                   fac2*( u(k)+u_stokes(k)     )
                Vr_sgwbl=fac1*( v(k-1)+v_stokes(k-1) )+                 &
     &                   fac2*( v(k)+v_stokes(k)     )
#   else
                Ur_sgwbl=fac1*u(k-1)+fac2*u(k)
                Vr_sgwbl=fac1*v(k-1)+fac2*v(k)
#   endif
          ENDIF
        END DO 
!
...
!
        ELSEIF ( sg_zwbl.gt.z2 ) THEN
!
#   ifdef BEDLOAD_VANDERA_STOKES
              Ur_sgwbl=ubar+ubar_stokes
              Vr_sgwbl=vbar+vbar_stokes
#   else
              Ur_sgwbl=ubar
              Vr_sgwbl=vbar
#   endif
        END IF
!
      ELSEIF ( sg_zwbl(ng).lt.Zr ) THEN
        z1=MAX( 2.5_r8*d50/30.0_r8, zapp_loc )
        z2=Zr
!
        IF ( sg_zwbl(ng).lt.z1 ) THEN
!
! If chosen height is less than the bottom roughness
! perform linear interpolation.
!
...
!
#   ifdef BEDLOAD_VANDERA_STOKES
          Ur_sgwbl=fac*(u(1)+u_stokes(1))
          Vr_sgwbl=fac*(v(1)+v_stokes(1))
#   else
          Ur_sgwbl=fac*u(1)
          Vr_sgwbl=fac*v(1)
#   endif
        ELSEIF ( sg_zwbl(ng).gt.z1 ) THEN
!
! If chosen height is less than the bottom cell thickness
! perform logarithmic interpolation with bottom roughness.
!
...
!
#   ifdef BEDLOAD_VANDERA_STOKES
          Ur_sgwbl=fac2*(u(1)+u_stokes(1))
          Vr_sgwbl=fac2*(v(1)+v_stokes(1))
#   else
          Ur_sgwbl=fac2*u(1)
          Vr_sgwbl=fac2*v(1)
#   endif
      ...
      IF (Ur_sgwbl.eq.0.0_r8) THEN
        phic_sgwbl=pi*SIGN(1.0_r8,Vr_sgwbl)
      ELSE
        phic_sgwbl=ATAN2(Vr_sgwbl,Ur_sgwbl)
      ENDIF
! 
      RETURN 
      END SUBROUTINE 
