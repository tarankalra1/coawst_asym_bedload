...
#  endif
#  ifdef BEDLOAD_VANDERA_REGZR
! 
! Fixed wave boundary layer elevation. 
!
      real(r8), parameter :: sg_zwbl=0.1_r8
!
#  elif defined BEDLOAD_VANDERA_FIXZR
!
! Reference height used as input to compute
! Madsen based wave boundary layer elevation. 
!
!
      real(r8), parameter :: sg_z1min=0.1_r8
!
#  endif
!
...
# ifdef BEDLOAD_VANDERA_REGZR
!
! Use Zr as above so no changes to Zr. 
!
#  ifdef BEDLOAD_VANDERA_STOKES
          Ur_sg(i,j)=u(i,j,1,nrhs)+u_stokes(i,j,1)
          Vr_sg(i,j)=v(i,j,1,nrhs)+v_stokes(i,j,1)
#  endif 
!
# elif defined BEDLOAD_VANDERA_FIXZR
!
...
!
            ...
            ksd_wbl(i,j)=m_zoa
!
! current shear stress can be negative 
! but udelta_wbl needs to be positive, 
! the bedload calculation takes care of the
! sign of udelta, so use Absolute value
!
            ustrc_wbl(i,j)=ABS(m_ustrc)
!
# if defined BEDLOAD_VANDERA_FIXZR 
!
! Use Madsen calculated thickness of wave boundary layer.
!
           thck_wbl(i,j)=m_dwc
           cff=LOG(MAX((thck_wbl(i,j)/ksd_wbl(i,j)),1.0_r8 ))
!
# elif defined BEDLOAD_VANDERA_REGZR 
!
! Use a user defined, fixed thickeness of wave boundary layer. 
! 
            thck_wbl(i,j)=sg_zwbl  ! Fixed 
            cff=LOG(thck_wbl(i,j)/ksd_wbl(i,j))
# endif 
!
...
# ifdef JCW_BBLTHICK
        dwc(1)=MAX(zo,MIN(0.97_r8*Dstp,dwc(1)))
# endif
      ELSE
        dwc(1)=kN
      END IF
...
# ifdef JCW_BBLTHICK
          dwc(i)=MAX(zo,MIN(0.97_r8*Dstp,dwc(i)))
# endif
        ELSE
          dwc(i)=kN
        END IF
